...
def featureMatching(image_ref, image_cur, matching_algorithm, threshold_value, output_path):
    ...
    #Task1
    kp1, des1 = detector.detectAndCompute(image_ref,None)
    ...
    printMatchesToFile(match_list, output_path)
    ...
    #Task7
    img3 = cv2.drawMatchesKnn(image_ref, kp1, image_cur, kp2, match_list[:15], None, flags=2)
    fig = plt.figure()
...
#Task6
def toRound(d):
    return str(format(d.distance, '.2f'))
...
def printMatchesToFile(matches, output_path):
    ...
    for i in range(len(matches)):
        if len(matches[i]) == 0:
            ...
            distance = " ".join(map(toRound, list(matches[i])))
            output_file.write(f"{i}: {distance}\n")
